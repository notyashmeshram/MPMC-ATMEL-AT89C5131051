	...
	while(1)					// endless 
	{
		                                     ...
		                                     
		{
			for(i = 0; i < 10; i++)
			{
				...
				SPDAT= 0xF0;			// 80H written to start ADC CH0 single ended sampling(refer D0D1D2config diag.)
				while(!transmit_completed);	// wait end of transmition 
			...
			LCD_CmdWrite(0x89);		 //
			//voltage[3] = avgVal%10 + 0x30; // add 0x30 to convert the corresponding number to ASCII
			//avgVal = avgVal/10;
